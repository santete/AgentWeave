	...
	for (let k = 0; k < dong.length; k++) {
		...
		if (muc || so) {
			...
			if (!ds || ds.tagName.toLowerCase() !== loai) {
				...
				ds.className = "md-ds";
				// Giữ SỐ BẮT ĐẦU thật. Nếu model viết "3." sau một đoạn văn xen giữa,
				// để trình duyệt tự đánh lại từ 1 là bịa ra một thứ tự khác với thứ tự
				// model nói — người đọc sẽ trích dẫn nhầm mục.
				if (so) {
					const batDau = Number.parseInt(so[1], 10);
					if (batDau > 1) ds.start = batDau;
				}
		...
		// ── DÒNG TRỐNG GIỮA HAI MỤC KHÔNG ĐÓNG DANH SÁCH ──
		//
		// Đây là lỗi "toàn số 1" mà người dùng báo. Model viết danh sách kiểu
		// "thoáng" — mỗi mục cách nhau một dòng trống, rất hay gặp khi mục có dẫn
		// đề in đậm:
		//
		//     1. **Định dạng file:** ...
		//     <trống>
		//     2. **Cấu hình build:** ...
		//
		// Bản trước đóng danh sách ở BẤT KỲ dòng nào không phải mục, nên mỗi mục
		// thành một <ol> riêng chứa đúng một <li> — và <ol> nào cũng bắt đầu từ 1.
		// Model hoàn toàn không sai: vết tích cho thấy nó nhả ra đúng 1,2,3,4.
		//
		// Nhìn TỚI dòng không trống kế tiếp: cùng loại danh sách thì giữ mở.
		if (d.trim() === "" && ds) {
			let j = k + 1;
			while (j < dong.length && dong[j].trim() === "") j++;
			const tiep = j < dong.length ? dong[j] : "";
			const cungLoai =
				ds.tagName.toLowerCase() === "ul"
					? /^\s*[-*]\s+/.test(tiep)
					: /^\s*\d{1,3}[.)]\s+/.test(tiep);
			if (cungLoai) continue;
		}
...
window.addEventListener("message", (ev) => {
	...
	switch (e.type) {
		...
		case "model_switched":
			...
			themKhoi("he-thong", `🖼 ${e.to} đọc ảnh` + (e.tam ? ` → ${e.from} dựng theo mô tả` : ""));
			break;
		...
		case "turn_end": {
			...
			if (e.perf) {
				...
				if (p.tokPerSec != null) {
					bang.appendChild(
						oSo("⚡", `${p.tokPerSec.toFixed(1)} tok/s`, p.tokPerSec >= 30 ? "nhanh" : "cham"),
					);
				}
				if (p.ttftMs != null) {
					bang.appendChild(
						oSo("⏱ chờ", `${(p.ttftMs / 1000).toFixed(1)}s`, p.ttftMs > 5000 ? "cham" : ""),
					);
				}
				...
				bang.appendChild(
					oSo(
						"↕",
						`${e.usage.inputTokens.toLocaleString()}/${e.usage.outputTokens.toLocaleString()} tok`,
					),
				);
			...
			if (soSua > 0 && e.checkOutcome !== "dat") {
				...
				const b = document.createElement("button");
				b.textContent =
					e.checkOutcome === "hong" ? "Bảo agent sửa cho đạt" : "Bảo agent chạy kiểm tra";
				b.className = "phu";
				b.onclick = () => {
					const y =
						e.checkOutcome === "hong"
...
function gui() {
	...
	vscode.postMessage({
		type: "hoi",
		text: t,
		images: anhCho.map((a) => ({ data: a.data, mimeType: a.mimeType })),
	});
	xoaHetAnh();
}
...
function veBangPhien(ds, current) {
	...
	for (const p of ds) {
		...
		meta.textContent =
			`${luc} · ${p.soLuot} lượt · ${p.model}` + (p.id === current ? " · đang mở" : "");
		hang.appendChild(t);
